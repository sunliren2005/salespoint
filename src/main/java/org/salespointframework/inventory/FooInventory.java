 ...
 */
public interface FooInventory<T extends InventoryItem>
		extends FooAbstractInventory<InventoryItem>, CrudRepository<T, InventoryItemIdentifier> {

	// @Query("select i from #{#entityName} i where i.product.id = ?1")
	// Streamable<T> streamByProductIdentifier(ProductIdentifier productIdentifier);
